...
def prepare_dataset(description, raw_data):
    ...
    for row in raw_data:
        ...
        country_year_data = [{'year': int(year_), 'coverage': cov} for year_, cov in zip(description[1], year_data)]
...
def get_year_data_from_raw_dataset(raw_data, year, description=description):
    ...
    result = [(get_country(rd[0]), rd[1][year_idx])
              for rd in raw_data]
...
def get_country_data(prepared_dataset, country):
    country_data = [(x['year'], x['coverage'])
                    for x in prepared_dataset[country]]
...
def perform_average(some_list):
    ...
    coverages = [int(x[1]) if x[1] != ": "
                 else 0 for x in some_list]
    lung = len(coverages)
